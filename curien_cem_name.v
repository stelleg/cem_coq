...
- invert H2. invert H1. specialize (IHX1 _ _ _ _ e0 l H eq_refl eq_refl). unfold
  isfresh in i. apply env_eq_fresh with (x:=f) (c:=cl (close N e) ne) in IHX1;
  auto. eapply IHX2 in IHX1; auto. auto. Qed.
...
- inversion H. subst.  specialize (IHstep1 h (close m l)). destruct IHstep1.
  ...
  (fresh (domain conf_h)). specialize (IHstep2 ((x, cl (close n l) l0)::conf_h)
  (close b x)). destruct IHstep2. constructor. 
  apply env_eq_cons with (l':=l) (l'':=l0). unfold lookup.
  ...
  exists x0. destruct x0. destruct y. destruct conf_c. split. assert (db.value
  cl_tm). apply values_only in s0. auto. inversion H0. subst. eapply App; eauto.
  auto.  
Qed. 
